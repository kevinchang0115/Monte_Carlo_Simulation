...
% Date:        2016/5/16
% Purpose:     To use Monte Carlo simulation to determine the fluence rate
%              of the scattered photons for laser beams with a Gaussian 
%              distribution in a double-layer tissue model.

% Description: 
%     This program uses variable weight photons and anisotropic scattering
%     (Henyey-Greenstein phase function) to simulate the fluence rate for a
%     collimated beam that has a Gaussian distribution with a radius of 0.5
%     mm and total power of 1W.  After running this program, we will get
%     two plots. The first is the average absorbance, reflectance and
%     transmittance. The second is the subplot of six graphs of the fluence
%     rate. First five graphs are five runs of the simulation of 10000
%     photons, and the last one is the graph of average values.
...
%     Fgrid:  fluence rate of the grid (W/cm^2)
...
% plot the heat map of the fluence rate of scattered photons
for i = 1:run
    for j = 1:dnum
        for k = 1:rnum
            dv = (2*k-1)*pi*dz*dr^2;
            if dnum <= t1/dz
                Fgrid(i,j,k) = photon_power*Agrid(i,j,k)/dv/num/ua1;
            else
                Fgrid(i,j,k) = photon_power*Agrid(i,j,k)/dv/num/ua2;                
            end
        end
    end
end
